...
class Game {
	...
	constructor(
		...
		this.manager.send(whiteId, {
			type: INIT_GAME,
			payload: { color: "white" },
		});
		this.manager.send(blackId, {
			type: INIT_GAME,
			payload: { color: "black" },
		});
	}
	...
	makeMove(playerId: string, move: Move) {
		if (this.gameOver) {
			this.manager.send(playerId, {
				type: ERROR,
				payload: { message: "Game is already over" },
			});
			return { ok: false, message: "Game is already over" };
		}

		const expectedPlayer =
			this.board.turn() === "w" ? this.whiteId : this.blackId;
		...
		if (this.board.isGameOver()) {
			...
			this.manager.send(this.whiteId, { type: GAME_OVER, payload: { winner } });
			this.manager.send(this.blackId, { type: GAME_OVER, payload: { winner } });
...
const globalGameManager = globalThis as unknown as {
